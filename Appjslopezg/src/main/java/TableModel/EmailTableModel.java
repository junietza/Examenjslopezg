...
/**
 * La clase EmailTableModel es un modelo personalizado, que hereda de
 ...
 * correos nos apoyamos en el codigo de la clase DefaultTableModel, ya que
 ...
 * @author willj
 */
public class EmailTableModel extends AbstractTableModel implements Serializable {
    ...
    /**
     * En este constructor podemos pasarle el arreglo de correos que queremos
     * mostrar en la tabla y tambien cargamos el arreglo que contiene el nombre
     * de las columnas
     *
     * @param emails
     */
    public EmailTableModel(Email[] emails) {
        loadColumnNames();
        loadDataModel(emails);
    }

    /**
     * Este método nos permite pasarle un arreglo de correos y transformarlo a
     * matriz para poder mostrar su contenido en la tabla
     *
     * @param arreglo de Email
     */
    public void setDataModel(Email[] emails) {
        loadDataModel(emails);
    }
     ...
     * correos
     *
     * @return un arreglo de Email
     */
    public Email[] getDataModel() {
        Email[] emails = new Email[rowData.length];
        for (int i = 0; i < rowData.length; i++) {
            Object[] row = rowData[i];
            emails[i] = convertToEmail(row);
        }
        return emails;
    }

    /**
     * Este método convierte cada fila en un objeto de tipo Email, cada campo
     ...
     */
    private Email convertToEmail(Object[] row) {
        Email temp = new Email();
        temp.setType(String.valueOf(row[0]));
        temp.setEmail(String.valueOf(row[1]));
        return temp;
    }
    ...
    public static Object[] getDefaultRowData() {
        Email p = new Email();
        p.setType("Tipo ");
        p.setEmail("cuenta@dominio.com");
        productId++;

        return new Object[]{p.getType(), p.getEmail()};
    }
    ...
    /**
     * Devuelve el numero total de columnas de la tabla de correos
     *
     * @return un entero
     */
    @Override
    ...
    /**
     * Este método agrega una nueva fila a la matriz de correos.
     *
    ...
    /**
     * Este método obtiene el nombre de todos los campos de la clase Email y
    ...
    /**
     * Este método se encarga de convertir el arreglo de correos en filas de
     ...
     */
    private void loadDataModel(Email[] emails) {
        if (emails != null && emails.length > 0) {
            rowData = new Object[emails.length][columnNames.length];
            int index = 0;
            for (Email a : emails) {
     ...
     * clase Email en un arreglo de tipo Field[] despues vamos recorriendo
     ...
     */
    private Object[] convertToArray(Email product) {
        Field fields[] = Email.class.getDeclaredFields();
        ...
        for (Field f : fields) {
            ...
            try {
                ...
                Logger.getLogger(EmailTableModel.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        return row;
    }
