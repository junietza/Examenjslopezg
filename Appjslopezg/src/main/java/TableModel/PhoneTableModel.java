...
/**
 * La clase PhoneTableModel es un modelo personalizado, que hereda de
 ...
 * @author willj
 */
public class PhoneTableModel extends AbstractTableModel implements Serializable {
     ...
     * @param phones
     */
    public PhoneTableModel(Phone[] phones) {
        loadColumnNames();
        loadDataModel(phones);
    }
     ...
     *
     * @param arreglo de Phone
     */
    public void setDataModel(Phone[] phones) {
        loadDataModel(phones);
    }
     ...
     * teléfonos
     *
     * @return un arreglo de Phone
     */
    public Phone[] getDataModel() {
        Phone[] phones = new Phone[rowData.length];
        for (int i = 0; i < rowData.length; i++) {
            Object[] row = rowData[i];
            phones[i] = convertToPhone(row);
        }
        return phones;
    }

    /**
     * Este método convierte cada fila en un objeto de tipo Phone, cada campo
     ...
     */
    private Phone convertToPhone(Object[] row) {
        Phone temp = new Phone();
        temp.setType(String.valueOf(row[0]));
        temp.setNumber(String.valueOf(row[1]));
        return temp;
    }
    ...
    public static Object[] getDefaultRowData() {
        ...
        return new Object[]{p.getType(), p.getNumber()};
    }
    ...
    /**
     * Este método obtiene el nombre de todos los campos de la clase Phone y
     ...
     */
    private void loadDataModel(Phone[] phones) {
        if (phones != null && phones.length > 0) {
            rowData = new Object[phones.length][columnNames.length];
            int index = 0;
            for (Phone a : phones) {
     ...
     * clase Phone en un arreglo de tipo Field[] despues vamos recorriendo
     ...
     */
    private Object[] convertToArray(Phone product) {
        Field fields[] = Phone.class.getDeclaredFields();
        ...
        for (Field f : fields) {
            ...
            try {
                ...
                Logger.getLogger(PhoneTableModel.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        return row;
    }
